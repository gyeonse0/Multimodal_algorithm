        ...
        for route in self.routes:
            ...
            for idx in range(1, len(route)):
                if route[idx][1] == CATCH or ((route[idx][1] == FLY) and (route[idx-1][1] in [ONLY_TRUCK, ONLY_DRONE])):
                    ...
                    # CATCH 이후, 트럭의 서비스 시간, 즉 유휴 시간동안 충전해주는 로직
                    charging_energy = data["service_time"]/60 * (data["charging_kw_d"])

                    different_energy = data["battery_kwh_d"] - drone_kwh
                    #지금 만땅기준으로 드론이 부족한 양

                    if charging_energy > different_energy:
                        charging_energy = different_energy

                    if drone_kwh < data["battery_kwh_d"] and not last_catch: # 만땅이 아니고 마지막 운행이 아니라면
                    ...
                    if idx == len(route) - 1:
                        last = True
                    #드론 에너지 충전
                    charging_energy = ((data["edge_km_t"][route[idx-jump-1][0]][route[idx][0]]*data["curvy_road"]) / data["speed_t"])/60 * (data["charging_kw_d"])
                    #이게 그냥 이동거리 동안 충전을 다 해주는양
                    ...
                    if charging_energy > differnet_energy:
                        charging_energy = differnet_energy

                    if drone_kwh < data["battery_kwh_d"] and not last: # 만땅이 아니고 마지막 운행이 아니라면
                        drone_kwh += charging_energy #kwh 단위 고려
                        truck_kwh -= charging_energy #트럭에도 충전해주는 만큼 차감
                        
                        #우리는 근데 트럭에서 충전해주는 양은 cost에서 고려해주지 않는다.
                        #total_truck += charging_energy 
