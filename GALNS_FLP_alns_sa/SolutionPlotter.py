...
class SolutionSaver:
    ...
    def save_convergence_graph_by_time(self, interval, my_objectives, name):
        plt.figure(figsize=(10, 6))
        interval_min = [i / 60 for i in interval]
        plt.plot(interval_min, my_objectives, label='Current Objective by time')
        plt.plot(interval_min, np.minimum.accumulate(my_objectives), color='red', linestyle='-', label='Best Objective')
